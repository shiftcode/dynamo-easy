...
import { ModelWithMomentAsHashKey, ModelWithMomentAsIndexHashKey } from './models/model-with-moment-as-key.model'
import { ModelWithoutCustomMapper, ModelWithoutCustomMapperOnIndex } from './models/model-without-custom-mapper.model'
  ...
  describe('should map model', () => {
    describe('to db', () => {
      ...
      describe('model with non string/number/binary keys', () => {
        it('should accept date/moment as HASH or RANGE key', () => {
          const now = moment()
          const toDb: AttributeMap = Mapper.toDb(new ModelWithMomentAsHashKey(now), ModelWithMomentAsHashKey)
          expect(toDb.startDate.S).toBeDefined()
          expect(toDb.startDate.S).toEqual(now.utc().format())
        })
        it('should accept date/moment as HASH or RANGE key on GSI', () => {
          const now = moment()
          const toDb: AttributeMap = Mapper.toDb(new ModelWithMomentAsIndexHashKey(0, now))
          expect(toDb.creationDate.S).toBeDefined()
          expect(toDb.creationDate.S).toEqual(now.utc().format())
        })
