...
export function initOrUpdateProperty(
  ...
  const properties: PropertyMetadata<any>[] =
    Reflect.getMetadata(KEY_PROPERTY, target.constructor) || []
  let existingProperty: PropertyMetadata<any> = properties.find(
    property => property.name === propertyKey
  )

  if (existingProperty) {
    // merge property options
    Object.assign<PropertyMetadata<any>, Partial<PropertyMetadata<any>>>(
      existingProperty,
      propertyMetadata
    )
  } else {
    // add new options
    let newProperty: PropertyMetadata<any> = createNewProperty(
      propertyMetadata,
      target,
      propertyKey
    )
...
function createNewProperty(
  ...
  console.log(
    `#### propertyKey: ${propertyKey} / typeInfo: ${JSON.stringify(typeInfo)}`
  )
