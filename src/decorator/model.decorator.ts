...
export function Model<T>(opts: ModelData = {}): ClassDecorator {
  return (constructor: Function) => {
    ...
    // get partition key
    const partitionKeys = properties ? properties.filter(property => property.key && property.key.type === 'HASH') : null
    ...
    const transientProperties: string[] = properties && properties.length ? properties.filter(property => property.transient === true).map(property => property.name) : []

    const finalOpts = Object.assign<Partial<ModelMetadata<T>>, Partial<ModelMetadata<T>>, Partial<ModelMetadata<T>>>(
      {},
      {
        clazz: constructor,
        clazzName: type.name,
...
function getGlobalSecondaryIndexes(properties: Array<PropertyMetadata<any>>): Map<string, SecondaryIndex> | null {
  if (properties && properties.length) {
    return properties.filter(property => property.keyForGSI && Object.keys(property.keyForGSI).length).reduce((map, property: PropertyMetadata<any>): Map<
      string,
      SecondaryIndex
    > => {
      ...
      Object.keys(property.keyForGSI).forEach(indexName => {
        ...
        switch (property.keyForGSI[indexName]) {
          case 'HASH':
            if (gsi.partitionKey) {
              throw new Error(`there is already a partition key defined for global secondary index ${indexName} (property name: ${property.nameDb})`)
            }
          ...
          case 'RANGE':
            if (gsi.partitionKey) {
              throw new Error(`there is already a sort key defined for global secondary index ${indexName} (property name: ${property.nameDb})`)
            }
...
// TODO VALIDATION only 5 gsi are allowed per table
function getLocalSecondaryIndexes(basePartitionKey: string, properties: Array<PropertyMetadata<any>>): Map<string, SecondaryIndex> | null {
  if (properties && properties.length) {
    return properties.filter(property => property.sortKeyForLSI && property.sortKeyForLSI.length).reduce((map, property: PropertyMetadata<any>): Map<
      string,
      SecondaryIndex
    > => {
      ...
      property.sortKeyForLSI.forEach(indexName => {
        if (map.has(indexName)) {
          throw new Error(`only one sort key can be defined for the same local secondary index, ${property.nameDb} is already defined as sort key for index ${indexName}`)
        }

        if (!basePartitionKey) {
          throw new Error('a local secondary index requires the partition key to be defined, use the @PartitionKey decorator')
        }
