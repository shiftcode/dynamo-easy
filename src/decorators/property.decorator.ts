...
export function initOrUpdateProperty(propertyMetadata: Partial<PropertyMetadata> = {}, target: any, propertyKey: string): void {
  // Update the attribute array

  const properties: PropertyMetadata[] = Reflect.getMetadata(KEY_PROPERTY, target.constructor) || []
  let existingProperty: PropertyMetadata = properties.find(property => property.key === propertyKey)

  if (existingProperty) {
    // merge property options
    Object.assign<PropertyMetadata, Partial<PropertyMetadata>>(existingProperty, propertyMetadata)
  } else {
    // add new options
    let newProperty: PropertyMetadata = createNewProperty(propertyMetadata, target, propertyKey)
...
function createNewProperty(propertyOptions: Partial<PropertyMetadata> = {}, target: any, propertyKey: string): PropertyMetadata {
  ...
  console.log(`#### propertyKey: ${propertyKey} / propertyType: ${propertyType} / propertyTypeName: ${propertyTypeName}`)

  let propertyDescriptor: PropertyDescriptor = Reflect.getOwnPropertyDescriptor(target, propertyKey)
  propertyOptions = Object.assign<any, Partial<PropertyMetadata>, Partial<PropertyMetadata>>(
    {},
    {
      key: propertyKey,
      name: propertyKey,
