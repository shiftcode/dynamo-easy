...
export class Util {
  ...
  static detectCollectionType(collection: any[] | Set<any>): AttributeCollectionType | null {
    if (Array.isArray(collection)) {
      ...
      const firstValueType = (<Set<any>>collection).size ? (<Set<any>>collection).values().next().value : null
  ...
  static isSet(value: any): boolean {
    return (value.hasOwnProperty('name') && (<any>value).name === 'Set') || value instanceof Set
  }
